...
export function getAzureSpeech() {
...
// Acotaciones que se vuelven silencios reales en Azure. Las demás se quitan:
// las voces HD detectan la emoción del texto por sí solas.
const AZURE_PAUSAS = [
  [/\[pausa larga\]/gi, '<break time="1200ms"/>'],
  [/\[pausa( corta)?\]/gi, '<break time="600ms"/>'],
  [/\[(respira|suspiro)\]/gi, '<break time="800ms"/>'],
];

function textoAzureSsml(texto, voz) {
  const lang = /^([a-z]{2}-[A-Z]{2})/i.exec(voz)?.[1] || "es-MX";
  let t = texto.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  for (const [re, tag] of AZURE_PAUSAS) t = t.replace(re, tag);
  t = t.replace(/\[[^\]\n]{1,40}\]/g, "");
  return (
    `<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="${lang}">` +
    `<voice name="${voz}">${t}</voice></speak>`
  );
}

/** Sintetiza UN trozo con Azure Speech. Devuelve PCM crudo con el mismo formato
 *  que el motor Gemini ({ pcm, rate }) para reusar toda la tubería WAV/MP3. */
export async function sintetizarChunkAzure(texto, voz) {
  const az = getAzureSpeech();
  if (!az) throw new Error("El motor Azure no está configurado en el servidor.");
  let res;
  try {
    res = await fetch(`https://${az.region}.tts.speech.microsoft.com/cognitiveservices/v1`, {
      method: "POST",
      headers: {
        "Ocp-Apim-Subscription-Key": az.key,
        "Content-Type": "application/ssml+xml; charset=utf-8",
        "X-Microsoft-OutputFormat": "raw-24khz-16bit-mono-pcm",
        "User-Agent": "voz-texto",
      },
      body: textoAzureSsml(texto, voz),
    });
  } catch {
    throw new Error("No se pudo conectar con Azure. Revisa tu internet e intenta de nuevo.");
  }
  if (!res.ok) {
    const detalle = (await res.text().catch(() => "")).slice(0, 120);
    throw new Error(`Azure ${res.status}${detalle ? ": " + detalle : ""}`);
  }
  const pcm = Buffer.from(await res.arrayBuffer());
  if (pcm.length < 1000) throw new Error("Azure no devolvió audio. Intenta de nuevo.");
