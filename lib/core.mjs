...
/** El candado: cada petición debe traer la cabecera x-clave con la contraseña.
 *  Si NO hay contraseña configurada, bloquea todo (fail-closed): así el sitio
 *  desplegado sin variables de entorno queda cerrado y nadie gasta las llaves. */
export function checkAuth(req) {
  const pass = getPassword();
  if (!pass) return false;
  const h = req.headers["x-clave"] || req.headers["X-Clave"] || "";
  return String(h) === pass;
}
